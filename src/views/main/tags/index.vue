  ...
  export default {
    ...
    methods: {
      ...
      closePage(event, name) {
        ...
        let lastVisitedView = visitedViews[0];

        if (this.currentPageName === name) {
          let len = visitedViews.length;
          for (let i = 1; i < len; i++) {
            if (visitedViews[i].name === name) {
              if (i < (len - 1)) {
                lastVisitedView = visitedViews[i + 1];
              } else {
                lastVisitedView = visitedViews[i - 1];
              }
              break;
        ...
        this.$store.dispatch('delVisitedView', name);
        visitedViews = this.$store.getters.visitedViews;
        if (this.currentPageName === name) {
          this.linkTo(lastVisitedView);
        }
      },
      ...
      handleTagsOption(type) {
        if (type === 'clearAll') {
          ...
          let currentName = this.$router.name;
          this.$store.dispatch('delOtherVisitedViews', currentName);
          let firstVisitedView = this.visitedViews[0];
          this.linkTo(firstVisitedView)
        }
        this.tagBodyLeft = 0;
      },
